...
- [ ] 2.1 `__root.tsx`: exact redacted layout — fullscreen routes (/welcome /setup /join) render bare; everything else inside canvas column (max-w-md) + BottomNav. NO header chrome
- [ ] 2.2 BottomNav component: copy redacted's (icon tabs, press-pop, active fill), adjust `match()` to our routes: Home↔/dashboard /allowance /chores; Family↔/family; Me↔/me /stash-remnants
